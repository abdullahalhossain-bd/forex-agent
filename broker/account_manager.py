...
# FIX (2026-08-08, EURNOK trading_permission audit): this used to be a
# SEPARATE hardcoded copy of the same table in data/live_feed.py — only
# 7 of the 48 symbols in config.SYMBOLS had a real limit here, so every
# Nordic/exotic cross (EURNOK, GBPNOK, EURSEK, GBPSEK, USDTRY, USDZAR,
# ...) silently fell back to DEFAULT=10.0 sized for majors. That's why
# EURNOK kept failing here (stage=trading_permission) even after the
# equivalent table in live_feed.py was fixed — this was a second,
# independent copy nothing else pointed at. Now imports the single
# maintained table from data.live_feed so the two gates can never drift
# apart again; falls back to a local copy only if that import fails.
try:
    from data.live_feed import SPREAD_LIMITS_PIPS
except Exception:
    log.warning(
        "[AccountManager] could not import SPREAD_LIMITS_PIPS from "
        "data.live_feed — using local fallback table (majors only, "
...
class AccountManager:
    ...
    def get_account_snapshot(self) -> dict | None:
        ...
        snapshot = {
            ...
            # MT5 User Guide Page 6 — account type (netting vs hedging)
            # Critical for position management: netting merges same-symbol orders,
            # hedging keeps them as independent positions.
            "margin_mode":     self._get_margin_mode(account),
            "account_type":    self._get_account_type_label(account),
            "checked_at":      datetime.now(timezone.utc).isoformat(timespec="seconds"),
        }
        snapshot["status"] = self._classify_health(snapshot)
        return snapshot
 
    def _get_margin_mode(self, account) -> str:
        """MT5 User Guide Page 6 — detect netting vs hedging account type.
        Returns: 'netting' | 'hedging' | 'unknown'
        """
        try:
            # MT5 account_info has 'margin_mode' field:
            #   0 = ACCOUNT_MARGIN_MODE_RETAIL_NETTING
            #   1 = ACCOUNT_MARGIN_MODE_RETAIL_HEDGING
            #   2 = ACCOUNT_MARGIN_MODE_EXCHANGE
            mode = getattr(account, "margin_mode", None)
            if mode is None:
                return "unknown"
            if mode == 0:
                return "netting"
    ...
    def _get_account_type_label(self, account) -> str:
        """Human-readable account type label for logging."""
        mode = self._get_margin_mode(account)
        if mode == "netting":
            return "Netting (orders merge into single position per symbol)"
        elif mode == "hedging":
            return "Hedging (independent positions, can hold opposite directions)"
        elif mode == "exchange":
            return "Exchange (exchange-traded instruments)"
        return f"Unknown ({mode})"
