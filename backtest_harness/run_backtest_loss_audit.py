    ...
    while i < n - 1:
        ...
        if open_trade is not None:
            ...
            if outcome is not None:
                ...
                extra = {k: open_trade[k] for k in (
                    "net_score", "bull_score", "bear_score", "n_signals",
                    "regime_direction", "regime_strength", "regime_type", "regime_volatility",
                    "adx", "ema50_gt_ema200", "ema200_slope_atr", "dist_from_ema200_atr",
                    "rsi", "macd_hist", "structure_bias", "session", "spread", "signals_raw",
                ) if k in open_trade}
                trades.append({
        ...
        if use_primary:
            ...
            if p_signal in ("BUY", "STRONG_BUY", "SELL", "STRONG_SELL") and p_conf >= MIN_CONFIDENCE:
                ...
                if valid:
                    # ---- regime feature snapshot at signal time (for the
                    # regime-conditional BUY/SELL audit) ----
                    try:
                        struct_result = struct_engine.analyze(sub_p)
                        structure_bias = struct_result.get("structure", "UNKNOWN")
                    except Exception:
                        structure_bias = "UNKNOWN"
                    ema200_series = sub_p["ema_200"].dropna()
                    ema200_slope = (
                        (ema200_series.iloc[-1] - ema200_series.iloc[-11]) / atr_val
                        if len(ema200_series) >= 11 and atr_val else 0.0
                    )
                    hour = pd.Timestamp(bar["datetime"]).hour
                    if 0 <= hour < 7: session = "ASIAN"
                    elif 7 <= hour < 12: session = "LONDON"
                    elif 12 <= hour < 16: session = "LONDON_NY_OVERLAP"
                    ...
                    open_trade = {
                        ...
                        "net_score": p_result.get("net_score", 0),
                        "bull_score": p_result.get("bull_score", 0),
                        "bear_score": p_result.get("bear_score", 0),
                        "n_signals": len(p_result.get("signals", []) or []),
                        "regime_direction": regime_result.get("direction", "UNKNOWN"),
                        "regime_strength": regime_result.get("strength", "UNKNOWN"),
                        "regime_type": regime_result.get("regime", "UNKNOWN"),
                        "regime_volatility": regime_result.get("volatility", "UNKNOWN"),
                        "adx": ind_ctx.get("adx", 0),
                        "ema50_gt_ema200": bool(ind_ctx.get("ema_50", 0) > ind_ctx.get("ema_200", 0)),
                        "ema200_slope_atr": round(float(ema200_slope), 3),
                        "dist_from_ema200_atr": round(
                            (ind_ctx.get("price", 0) - ind_ctx.get("ema_200", 0)) / atr_val, 3
                        ) if atr_val else 0.0,
                        "rsi": ind_ctx.get("rsi", 50),
                        "macd_hist": round(float(ind_ctx.get("macd", 0) - ind_ctx.get("macd_signal", 0)), 6),
                        "structure_bias": structure_bias,
                        "session": session,
                        "spread": float(bar["spread"]) if not pd.isna(bar["spread"]) else 0.0,
                        "signals_raw": "|".join(
                            f"{d}:{w}:{desc}" for d, w, desc in (p_result.get("signals") or [])
                        ),
                    }
                    opened = True
