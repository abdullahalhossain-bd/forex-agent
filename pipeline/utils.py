...
# ── Symbol universe ────────────────────────────────────────────
_FALLBACK_SYMBOLS = ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "XAUUSD"]


def _load_default_symbols() -> List[str]:
    """Pull the full pair universe from the project's config.py (SYMBOLS list)
    so the pipeline trains on the same symbols the live bot trades — instead
    of the old hardcoded 6-pair list. Falls back to that 6-pair list only if
    config.py can't be imported (e.g. pipeline run standalone/in tests)."""
    try:
        if str(PROJECT_ROOT) not in sys.path:
            sys.path.insert(0, str(PROJECT_ROOT))
        from config import SYMBOLS  # project-root config.py — single source of truth
        if SYMBOLS:
            return list(SYMBOLS)
    except Exception as e:
        logging.getLogger("pipeline.utils").warning(
            f"Could not import SYMBOLS from project config.py ({e}); "
            f"falling back to {_FALLBACK_SYMBOLS}"
        )
...
class PipelineConfig:
    ...
    # Symbols — defaults to the full universe defined in config.py
    symbols: List[str] = field(default_factory=_load_default_symbols)
