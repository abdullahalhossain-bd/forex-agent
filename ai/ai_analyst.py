...
class AIAnalyst:
    ...
    def _call_with_timeout(self, fn, *args, timeout: float, **kwargs):
        ...
        *waiting* on the future — it does NOT cancel the underlying thread.
        ...
        try:
            ...
            # Demoted to INFO: timeouts are the *expected* outcome when
            # a cloud provider is slow or unreachable. The cascade falls
            # through to the next provider cleanly.
            # To reduce noise: only log at INFO level. The previous
            # WARNING level produced a noisy log line per cycle for a
            # condition that's not actionable mid-run.
            fn_name = getattr(fn, '__name__', 'LLM call')
            log.info(f"[AIAnalyst] {fn_name} timed out after {timeout}s")
            return None
    ...
    def analyze(
        ...
        **kwargs,
        ...
        # design was based on the assumption that a local model would be
        # cheaper/faster, but in practice:
        #   - On Windows laptops without Ollama installed, every cycle
        #     wasted 5-15 seconds timing out on localhost:11434 before
        #     the cloud cascade could even start.
        #   - The local model (qwen3:4b) was weaker than cloud models at
        #     trade-reasoning JSON, producing more parse failures.
        #   - Cloud providers (Groq free tier) are fast enough and far
        #     more reliable for structured-output prompts.
        #
        # Order rationale:
        #   1. Groq      — fastest (LPU inference), free tier, primary
        #   2. Gemini    — most reliable free-tier cloud after Groq
        #   3. OpenRouter — routes to 100+ models, multiple free options
        #   4. Cerebras  — optional, only if user explicitly configured
        #   5. SambaNova — optional, only if user explicitly configured
        # (Cerebras/SambaNova are kept as opt-in extras — both have
        #  known breakage as of 2026-Q3 — but disabled by default.)
        raw = None
        ...
        # ── 2. FALLBACK #1: Gemini ─────────────────────────────────
        # Most reliable free-tier provider after Groq. Cerebras (Cloudflare
        # 403) and SambaNova (410 Gone) are known broken, so we try Gemini
        # BEFORE them to avoid wasted latency.
        ...
        # ── 4. OPTIONAL EXTRAS: Cerebras / SambaNova ───────────────
        # Disabled by default — both have known breakage as of 2026-Q3
        # (Cerebras: Cloudflare 403; SambaNova: 410 Gone for Llama 3.1).
        # To enable: set OC_INCLUDE_CEREBRAS=1 / OC_INCLUDE_SAMBANOVA=1
        # in .env. These are kept as opt-in extras for users who have
        # working credentials and want maximum fallback depth.
        _include_cerebras = os.getenv("OC_INCLUDE_CEREBRAS", "0") == "1"
        _include_sambanova = os.getenv("OC_INCLUDE_SAMBANOVA", "0") == "1"

        if raw is None and _include_cerebras and self._key_manager is not None and time.monotonic() < deadline:
        ...
        if raw is None and _include_sambanova and self._key_manager is not None and time.monotonic() < deadline:
...
{{
  ...
  "risk_warning": "Any additional risk warning for the trader"
    ...
    # ── 2026-07-25: Ollama methods removed ───────────────────────
    # The _get_ollama_client() and _call_ollama() methods used to live
    # here. They have been REMOVED because Ollama is no longer in the
    # provider cascade. The stubs below keep backward-compat with any
    # external caller (dashboards, tests) that pokes them directly.
    ...
    def _strip_ollama_thinking(text: str) -> str:
        ...
        Kept as a utility — still used by _parse_response() in case a
        cloud provider ever emits similar wrapping.
        """
