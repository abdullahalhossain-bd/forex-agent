...
class PositionAllocator:
    ...
    def __init__(
        ...
        min_rr: float = None,            # Minimum R:R for any trade (None -> resolved from rr_policy)
    ):
        from risk.rr_policy import get_min_rr
        self.balance = balance
        self.kelly_fraction = kelly_fraction
        # Was hardcoded 1.5, drifted from core/constants.MIN_RR_PROD=2.0.
        # Now resolves from the single source of truth unless a caller
        # explicitly overrides it.
        self.min_rr = min_rr if min_rr is not None else get_min_rr()
