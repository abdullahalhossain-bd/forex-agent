#!/usr/bin/env python3
"""
Forex-Agent Strategy v9 — Edge-Based Final
============================================
Based on forward-return analysis, BOS in trend direction has a small but
real edge (~52% WR with +2 pips avg at 20 bars). This strategy:
  1. Uses BOS as primary trigger (validated edge)
  2. Requires HTF trend alignment (EMA200)
  3. Uses ATR-based SL with swing low/high refinement
  4. Fixed 1:1.5 R:R (smaller TP = higher hit rate)
  5. Breakeven at 0.5R + profit lock at 1R
  6. Cooldown of 3 bars
  7. Allows up to 2 simultaneous signals (but we'll keep 1)
"""
...
def signal_v9(row, symbol='EURUSD') -> Dict[str, Any]:
    """v9 — edge-validated signal."""
    bull_score = 0
    bear_score = 0
    signals = []
    ...
    # CONFIRM: trend direction must match BOS
    if bos_bull and htf_bull:
        bull_score += 4; signals.append(('bullish', 4, 'Bullish BOS in uptrend'))
    elif bos_bear and htf_bear:
        bear_score += 4; signals.append(('bearish', 4, 'Bearish BOS in downtrend'))
    else:
        return {'signal': 'WAIT', 'confidence': 0, 'net': 0}  # BOS against trend = skip

    # CONFIRM: EMA stack alignment
    if htf_bull and ema9 > ema20 > ema50:
        bull_score += 2; signals.append(('bullish', 2, 'EMA stack bull'))
    elif htf_bear and ema9 < ema20 < ema50:
        bear_score += 2; signals.append(('bearish', 2, 'EMA stack bear'))

    # CONFIRM: MACD direction
    if htf_bull and row['macd'] > row['macd_signal']:
        bull_score += 1
    elif htf_bear and row['macd'] < row['macd_signal']:
        bear_score += 1

    # CONFIRM: RSI not extreme
    if htf_bull and 40 <= rsi_val <= 70:
        bull_score += 1
    elif htf_bear and 30 <= rsi_val <= 60:
        bear_score += 1
    ...
    # Threshold: net >= 6 for entry (4 from BOS + 2+ from confirms)
    if net >= 7 and htf_bull:
        signal = 'STRONG_BUY' if net >= 9 else 'BUY'
    elif net <= -7 and htf_bear:
        signal = 'STRONG_SELL' if net <= -9 else 'SELL'
    else:
...
def run_backtest_v9(
    ...
    atr_sl_mult: float = 1.2,        # moderate SL
    atr_tp_mult: float = 1.8,        # 1:1.5 R:R
    max_hold_bars: int = 30,
    strategy_name: str = 'v9_edge',
    enable_breakeven: bool = True, breakeven_at_r: float = 0.5,
    enable_lock_profit: bool = True, lock_profit_at_r: float = 1.0,
    lock_profit_sl_r: float = 0.5,
    enable_session_filter: bool = True, cooldown_bars: int = 3,
            ...
            if sig['signal'] in ('BUY', 'STRONG_BUY'):
                ...
                if use_swing_stops:
                    ...
                    if swing_low is not None and swing_low < actual_entry:
                        ...
                        if actual_entry - sl < 0.5 * atr_val:
                            sl = actual_entry - 0.5 * atr_val
                    else:
                        sl = actual_entry - atr_sl_mult * atr_val
                else:
                    sl = actual_entry - atr_sl_mult * atr_val
                ...
                if use_swing_stops:
                    ...
                    if swing_high is not None and swing_high > actual_entry:
                        ...
                        if sl - actual_entry > 2.0 * atr_val:
                            sl = actual_entry + atr_sl_mult * atr_val
                        if sl - actual_entry < 0.5 * atr_val:
                            sl = actual_entry + 0.5 * atr_val
                    else:
                        sl = actual_entry + atr_sl_mult * atr_val
                else:
...
def main():
    ...
    print("="*70)
    print("  v9 EDGE-BASED FINAL STRATEGY")
    print("  Primary trigger: BOS in trend direction (validated edge)")
    print("="*70)
    ...
    for symbol in symbols:
        ...
        print(f"\n{'='*60}")
        print(f"  v9 backtest for {symbol} H1")
        print(f"{'='*60}")

        df = prepare_data_v2(csv_path, symbol)
        result = run_backtest_v9(
            df=df, signal_fn=signal_v9,
            symbol=symbol, strategy_name='v9_edge',
        )
    ...
    metrics_csv = OUTPUT_DIR / "v9_edge_metrics.csv"
    trades_csv  = OUTPUT_DIR / "v9_edge_trades.csv"
    metrics_df.to_csv(metrics_csv, index=False)
    trades_df.to_csv(trades_csv, index=False)

    print(f"\n{'='*60}")
    print(f"  COMBINED v9 EDGE-BASED RESULTS  (7 pairs)")
    print(f"{'='*60}")
    if all_metrics:
        ...
        total_slp    = sum(m['exit_sl_profit'] for m in all_metrics)
        print(f"  Total trades: {total_trades}")
        print(f"  Combined WR: {total_wins/total_trades*100:.2f}%")
        print(f"  Total PnL: ${total_pnl:.2f}")
        print(f"  Exits: TP={total_tp} SL={total_sl} SL_BE={total_be} SL_PROFIT={total_slp}")
