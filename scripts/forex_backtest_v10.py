#!/usr/bin/env python3
"""
Forex-Agent Strategy v10 — Final Practical
============================================
After extensive testing, found that:
  - Simple indicators (BOS, EMA, RSI, MACD) have NO predictive edge on H1
  - Forward returns after any signal are ~50/50 (random walk)
  - Only way to be profitable: strong risk management + tiny edges
  
This strategy:
  1. Uses multi-factor confluence (5+ factors required)  
  2. Strict HTF gate (EMA200)
  3. ADX > 28 (strong trends only)
  4. Tight SL (1.0 ATR) with swing-based refinement
  5. Asymmetric R:R 1:1.5 (TP closer = higher hit rate)
  6. Aggressive breakeven (0.5R) + profit lock (1R)
  7. Confidence-weighted sizing
  8. Cooldown 5 bars
  9. Skip news bars (range > 2x avg)
  10. Session filter (London/NY only)

GOAL: Win rate 55%+ with positive expectancy after costs.
"""
...
def signal_v10(row, symbol='EURUSD') -> Dict[str, Any]:
    """v10 — Multi-factor confluence with strict filters."""
    bull_score = 0
    bear_score = 0
    bull_factors = 0
    bear_factors = 0
    ...
    # FACTOR 1: HTF trend (1 factor, weight 2)
    if htf_bull:
    ...
    # FACTOR 3: RSI in trend-aligned zone (momentum confirmation, not overbought/oversold)
    if htf_bull and 45 <= rsi_val <= 65:
        bull_score += 2; bull_factors += 1
    elif htf_bear and 35 <= rsi_val <= 55:
        bear_score += 2; bear_factors += 1

    # FACTOR 4: MACD momentum aligned
    ...
    # FACTOR 5: BOS in trend direction
    ...
    # FACTOR 10: BB position (trend, not mean reversion)
...
def run_backtest_v10(
    ...
    atr_tp_mult: float = 1.5,        # 1:1.5 R:R
    max_hold_bars: int = 30,
    strategy_name: str = 'v10_final',
            ...
            if sig['signal'] in ('BUY', 'STRONG_BUY'):
                ...
                if use_swing_stops:
                    ...
                    if swing_low is not None and swing_low < actual_entry:
                        ...
                        if actual_entry - sl > 1.5 * atr_val:
                ...
                if use_swing_stops:
                    ...
                    if swing_high is not None and swing_high > actual_entry:
                        ...
                        if sl - actual_entry > 1.5 * atr_val:
                            sl = actual_entry + atr_sl_mult * atr_val
...
def main():
    ...
    print("="*70)
    print("  v10 FINAL PRACTICAL STRATEGY")
    print("  Strict confluence + Risk management + BE + Profit Lock")
    print("="*70)
    ...
    for symbol in symbols:
        ...
        print(f"\n{'='*60}")
        print(f"  v10 backtest for {symbol} H1")
        print(f"{'='*60}")

        df = prepare_data_v2(csv_path, symbol)
        result = run_backtest_v10(
            df=df, signal_fn=signal_v10,
            symbol=symbol, strategy_name='v10_final',
        )
    ...
    metrics_csv = OUTPUT_DIR / "v10_final_metrics.csv"
    trades_csv  = OUTPUT_DIR / "v10_final_trades.csv"
    metrics_df.to_csv(metrics_csv, index=False)
    trades_df.to_csv(trades_csv, index=False)

    print(f"\n{'='*60}")
    print(f"  COMBINED v10 FINAL RESULTS  (7 pairs)")
    print(f"{'='*60}")
    if all_metrics:
