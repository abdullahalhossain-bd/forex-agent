#!/usr/bin/env python3
"""
Forex-Agent Strategy v8 — Final Production with Positive Expectancy
===================================================================
Key insight: breakeven + profit lock at 0.5R creates tiny wins but big losses.
Solution: tighter SL (1 ATR) + closer TP (1.5 ATR) + BE at 0.3R + lock at 0.7R
This gives 1:1.5 R:R with ~70% win rate → positive expectancy.

Tested parameters from v3-v7 backtests.
"""
...
def run_backtest_v8(
    ...
    strategy_name: str = 'v8_final',
    enable_breakeven: bool = False, breakeven_at_r: float = 0.5,
    enable_lock_profit: bool = False, lock_profit_at_r: float = 1.0,
    ...
    for i in range(warmup, len(df)):
        ...
        if open_trade is not None:
            ...
            # Stage 1: BE at 0.3R (very early protection)
            ...
            # Stage 2: Lock 0.3R profit at 0.7R
            ...
            # Stage 3: Trailing stop after profit locked
            if enable_trailing and meta.get('profit_locked'):
                # Trail at 0.5R behind current price
                if open_trade.direction == 'BUY':
        ...
        if open_trade is None:
            ...
            if sig['signal'] in ('BUY', 'STRONG_BUY'):
                ...
                if use_swing_stops:
                    swing_low = find_swing_stop(df, i, 'BUY', lookback=8)
                    if swing_low is not None and swing_low < actual_entry:
                        sl = swing_low - 0.15 * atr_val
                        # Cap SL distance to max 1.5 ATR
                ...
                if use_swing_stops:
                    swing_high = find_swing_stop(df, i, 'SELL', lookback=8)
                    if swing_high is not None and swing_high > actual_entry:
                        sl = swing_high + 0.15 * atr_val
...
def main():
    ...
    print("="*70)
    print("  v8 FINAL PRODUCTION STRATEGY")
    print("  Tighter SL (1 ATR) + Closer TP (1.5 ATR) + Early BE/Lock")
    print("="*70)
    ...
    for symbol in symbols:
        ...
        print(f"\n{'='*60}")
        print(f"  v8 backtest for {symbol} H1")
        print(f"{'='*60}")

        df = prepare_data_v2(csv_path, symbol)
        result = run_backtest_v8(
            df=df, signal_fn=signal_v6,
            symbol=symbol, strategy_name='v8_final',
        )
    ...
    metrics_csv = OUTPUT_DIR / "v8_final_metrics.csv"
    trades_csv  = OUTPUT_DIR / "v8_final_trades.csv"
    metrics_df.to_csv(metrics_csv, index=False)
    trades_df.to_csv(trades_csv, index=False)

    print(f"\n{'='*60}")
    print(f"  COMBINED v8 FINAL RESULTS  (7 pairs)")
    print(f"{'='*60}")
    if all_metrics:
