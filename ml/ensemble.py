...
class EnsembleEngine:
    ...
    def decide(
        ...
        #
        # AUDIT FIX: this was hardcoded to 45.0 with a comment "lowered from
        # 50 for better trade frequency" — but ml/model_weights.json's
        # `_thresholds.min_confidence` (the single declared source of truth
        # for this value, per that file's own comments) says 50. No
        # walk-forward/OOS evidence was attached to the 45 change; "more
        # trades" is not itself evidence of better risk-adjusted edge — it's
        # exactly the kind of gate-loosening that inflates trade count while
        # quietly admitting lower-quality signals. Reading from config so
        # there is one number, not two silently diverging ones. If you want
        # 45%, change it in model_weights.json with backtest evidence
        # attached, not as a second hardcoded value here.
        try:
            _weights_cfg = json.loads(
                (Path(__file__).resolve().parent / "model_weights.json").read_text(encoding="utf-8")
            )
            min_conf = float(_weights_cfg.get("_thresholds", {}).get("min_confidence", 50.0))
        except Exception:
            min_conf = 50.0  # conservative fallback if config is unreadable
