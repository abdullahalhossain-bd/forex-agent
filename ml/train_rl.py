...
def build_features_df(df: pd.DataFrame, pair: str) -> pd.DataFrame:
    ...
    BUG FIX: this used to `continue` (skip) the first 4 warm-up rows
    (wherever `len(sub_df) < 5`) instead of emitting a row for them.
    That silently shifted every later feature row 4 candles out of sync
    with `df` — `features_df.iloc[k]` actually described `df.iloc[k+4]`.
    Downstream, `ForexTradingEnv._get_state()` indexes both frames by the
    same positional `current_step`, so the agent was being shown feature
    vectors for the WRONG candle on every single step, not just during
    curriculum splitting. Fix: always emit exactly `len(df)` rows, using
    an all-zero placeholder for the warm-up period (matching how
    `ForexTradingEnv` already zero-fills missing state), so
    `features_df.iloc[k]` <-> `df.iloc[k]` holds by construction.
    """
    try:
        from ml.feature_engineer import get_feature_engineer
        engineer = get_feature_engineer()
        n = len(df)
        rows: List[Optional[Dict[str, Any]]] = [None] * n
        for i in range(n):
            sub_df = df.iloc[:i + 1]
            if len(sub_df) < 5:
                continue  # not enough history yet — leave as None, filled below
            rows[i] = engineer.build_feature_vector(
                df=sub_df, analysis_out={}, pair=pair, timeframe="15m",
            )

        template = next((r for r in rows if r is not None), None)
        if template is None:
            return pd.DataFrame()

        zero_row = {k: 0.0 for k in template.keys()}
        filled = [r if r is not None else dict(zero_row) for r in rows]
        features_df = pd.DataFrame(filled)
        features_df.index = pd.RangeIndex(n)  # positional, 1:1 with df
        return features_df
    except Exception as e:
...
def split_by_regime(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    """Return a boolean mask per curriculum stage (aligned to df's
    positional row order — index 0..len(df)-1), NOT a filtered copy of
    df.

    BUG FIX: this previously returned already-filtered DataFrames
    (`df[df["adx"] > 25].copy()`, etc). Since the ADX/ATR conditions
    select a non-contiguous subset of rows, callers had no way to slice
    `features_df` to match — the old code in `train_rl_agent` sliced
    `features_df.iloc[:len(stage_df)]` (the first N rows of the FULL,
    unfiltered feature set), which lines up with the filtered `stage_df`
    only by row COUNT, not by which candles they actually are. For 3 of
    4 curriculum stages (trend/range/volatile — 70% of training
    timesteps) the agent was trained against features describing
    entirely different candles than the price/PnL it was simulating and
    being rewarded on. Returning a mask lets the caller apply the exact
    same boolean selection to both `df` and `features_df`, guaranteeing
    alignment regardless of contiguity.
    """
    n = len(df)
    masks: Dict[str, np.ndarray] = {}
    if n == 0:
        return {stage.name: np.zeros(0, dtype=bool) for stage in CURRICULUM}

    try:
        if "adx" in df.columns:
            adx = df["adx"].to_numpy()
            masks["trend_market"] = adx > 25
            masks["range_market"] = adx <= 20
            if "atr" in df.columns:
                atr = df["atr"].to_numpy()
                atr_median = np.nanmedian(atr)
                masks["volatile_market"] = atr > (atr_median * 1.5)
            else:
                masks["volatile_market"] = np.ones(n, dtype=bool)
            masks["full_mixed"] = np.ones(n, dtype=bool)
        else:
            # No regime info — use all data for all stages
            for stage in CURRICULUM:
                masks[stage.name] = np.ones(n, dtype=bool)
    except Exception:
        for stage in CURRICULUM:
            masks[stage.name] = np.ones(n, dtype=bool)
...
def train_rl_agent(
    ...
    # BUG FIX: masks (not pre-filtered frames) so df and features_df are
    # always sliced with the EXACT same boolean selection below — see
    # split_by_regime()/build_features_df() docstrings for why the old
    # `.iloc[:len(stage_df)]` approach paired features with the wrong
    # candles for every non-contiguous curriculum stage.
    if use_curriculum:
        masks = split_by_regime(df)
    else:
        masks = {"full_mixed": np.ones(len(df), dtype=bool)}
    ...
    for stage in CURRICULUM:
        if not use_curriculum and stage.name != "full_mixed":
            continue
        mask = masks.get(stage.name, np.ones(len(df), dtype=bool))
        stage_df = df[mask].reset_index(drop=True)
        ...
        # Build environment for this stage — SAME mask applied to
        # features_df as was applied to df above, so row k of each is
        # guaranteed to describe the same candle.
        stage_features = (
            features_df[mask].reset_index(drop=True)
