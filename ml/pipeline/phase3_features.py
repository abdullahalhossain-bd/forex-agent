...
def compute_features(config: Optional[PipelineConfig] = None) -> Dict[str, pd.DataFrame]:
    ...
    with PipelineTimer("Phase 3: Feature Engineering", log):
        ...
        for symbol in config.symbols:
            ...
            # Compute each feature set. Many indicators use .rolling()/.shift()
            # whose first few rows are legitimately NaN (not enough history
            # yet) -- comparing those against 0 in indicator formulas (RSI,
            # ADX, swing highs/lows, FVG, BOS/CHoCH, etc.) is correct behavior
            # but numpy warns on it. It's harmless (those warmup rows get
            # dropped a few lines down), so it's suppressed here rather than
            # patching every individual comparison across the feature set.
...
def _rsi(close, period=14):
    delta = close.diff().fillna(0.0)  # first row is NaN; fillna avoids NaN-vs-0 compare warning
...
def _adx(high, low, close, period=14):
    plus_dm = high.diff().fillna(0.0)  # first row is NaN; fillna avoids NaN compare warning
    minus_dm = (-low.diff()).fillna(0.0)
