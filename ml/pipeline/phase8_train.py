...
class _RLProgressCallback:
    """Periodic progress log for RL training. Without this, verbose=0 +
    no callback means model.learn() prints NOTHING for the entire run --
    500k timesteps can legitimately take 20-90+ minutes on CPU, and with
    zero output there's no way to tell "still training" apart from "hung"."""

    def __new__(cls, symbol: str, algo: str, total_timesteps: int, log_every: Optional[int] = None):
        ...
        class _Impl(BaseCallback):
            def __init__(self):
                ...
                self.log_every = log_every or max(5_000, total_timesteps // 20)
                self._last = 0
                self._start = None

            def _on_training_start(self) -> None:
                self._start = time.time()
                log.info(f"    {self.symbol} {self.algo}: RL training started ({self.total:,} timesteps)")
            ...
            def _on_step(self) -> bool:
                if self.num_timesteps - self._last >= self.log_every:
                    ...
                    remaining_min = (self.total - self.num_timesteps) / rate / 60 if rate > 0 else float("nan")
                    log.info(f"    {self.symbol} {self.algo}: {self.num_timesteps:,}/{self.total:,} "
                             f"({pct:.0f}%) -- {elapsed/60:.1f}m elapsed, ~{remaining_min:.1f}m remaining")
                return True
...
def _train_supervised(model_type: str, X_train, y_train, X_val, y_val, symbol: str, feature_cols: list, config: PipelineConfig) -> Optional[Dict]:
    ...
    if model_type == "xgboost":
        ...
        xgb_kwargs = dict(n_estimators=200, max_depth=6, learning_rate=0.05,
                          subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
                          eval_metric="mlogloss", verbosity=0)
        try:
            # xgboost >= 2.0: early_stopping_rounds is a constructor param
            model = XGBClassifier(**xgb_kwargs, early_stopping_rounds=20)
            model.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False)
        except TypeError:
            # xgboost < 2.0: early_stopping_rounds is a fit() kwarg instead
            model = XGBClassifier(**xgb_kwargs)
            model.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=20, verbose=False)
    elif model_type == "lightgbm":
        try:
            from lightgbm import LGBMClassifier
            # reg_lambda=1.0 explicitly set to match XGBoost's default L2 —
            # LightGBM's own default is reg_lambda=0.0, so without this it was
            # training with far less regularization than the other boosted model.
            model = LGBMClassifier(n_estimators=200, max_depth=6, learning_rate=0.05,
                                   subsample=0.8, colsample_bytree=0.8,
                                   reg_alpha=0.1, reg_lambda=1.0, verbose=-1)
            try:
                # lightgbm >= 3.3: early stopping via callbacks
                from lightgbm import early_stopping, log_evaluation
                model.fit(X_train, y_train, eval_set=[(X_val, y_val)],
                          callbacks=[early_stopping(20, verbose=False), log_evaluation(0)])
            except ImportError:
                # older lightgbm: early_stopping_rounds fit() kwarg
                model.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=20, verbose=False)
        except ImportError:
    ...
    # Report early-stopping outcome (how many of the 200 rounds it actually used)
    best_iter = (getattr(model, "best_iteration_", None) or getattr(model, "best_iteration", None)
                 or getattr(model, "get_best_iteration", lambda: None)())
    iter_note = f", stopped at round {best_iter}/200" if best_iter else ""
    log.info(f"    {symbol} {model_type}: accuracy={acc:.4f} ({elapsed:.1f}s{iter_note})")
    
    # Save model
    model_dir = MODEL_OUTPUT_DIR / symbol / model_type
    model_dir.mkdir(parents=True, exist_ok=True)
    
    # Save model — safe_pickle_dump (not raw pickle.dump) so it writes the
    # companion .meta hash file that Phase 11's safe_pickle_load requires to
    # verify integrity before loading. Without this, Phase 11 fails for
    # every model with "No metadata file -- cannot verify integrity".
    from utils.safe_pickle import safe_pickle_dump
    model_path = model_dir / "model.pkl"
    safe_pickle_dump(model, str(model_path), metadata={"model_type": model_type, "symbol": symbol})
...
def _train_rl(algo: str, train_df: pd.DataFrame, feature_cols: list, symbol: str, config: PipelineConfig) -> Optional[Dict]:
    ...
    model.learn(total_timesteps=config.rl_timesteps,
                callback=_RLProgressCallback(symbol, algo, config.rl_timesteps))
    elapsed = time.time() - start
