...
class ModelPredictor:
    ...
    def __init__(self):
        ...
        # FIX: one-time-warn cache for legacy schema mismatches. Without
        # this, the same warning ("legacy model schema mismatch expects 13
        # features, got 161") was logged on every single cycle for every
        # pair — 50+ warnings per minute in trader.log, all identical.
        # Each (pair, tf, model_type, expected_count, actual_count) tuple
        # is warned once, then silenced until the process restarts.
        self._warned_legacy_schema: set = set()
            ...
            try:
                ...
                if model_type != "lstm":
                    if expected_features:
                        ...
                        # FIX: deduplicate the warning per (pair, tf, model_type)
                        # to avoid spamming the log on every cycle.
                        warn_key = (pair, timeframe, model_type,
                                    expected_count, X.shape[1])
                        if warn_key not in self._warned_legacy_schema:
                            self._warned_legacy_schema.add(warn_key)
                            log.warning(
