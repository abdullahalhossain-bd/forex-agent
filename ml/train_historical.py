...
Usage:
    ...
    python -m ml.train_historical --per-symbol # One symbol fully at a time (low RAM, crash-safe)
"""
...
def _mark_phase_done(phase_num: int, config: PipelineConfig) -> None:
...
def _has_raw_data(config: PipelineConfig) -> bool:
    """Check if Phase 1 data exists on disk for EVERY currently configured
    symbol (for resume validation) -- not just any single one of them."""
    ...
    if missing:
        log.warning(f"  Raw data missing/insufficient for {len(missing)}/{len(config.symbols)} "
                    f"symbol(s), e.g. {missing[:5]}")
        return False
    return True


def _has_featured_data(config: PipelineConfig) -> bool:
    """Check if Phase 3 feature cache exists on disk for EVERY currently
    configured symbol -- not just any single one of them."""
    tf = config.primary_timeframe
    missing = []
    for symbol in config.symbols:
        ...
        if not ok:
            missing.append(symbol)
    if missing:
        log.warning(f"  Feature cache missing/insufficient for {len(missing)}/{len(config.symbols)} "
                    f"symbol(s), e.g. {missing[:5]}")
        return False
    return True


def _load_cached_features(config: PipelineConfig) -> Dict:
    """Reload each symbol's cached feature parquet from disk into memory.
    Needed when Phase 3 is skipped via checkpoint — otherwise downstream
    phases (4, 5, 6...) would see an empty featured_data dict and skip too,
    even though the data is sitting right there on disk."""
    import pandas as pd
    tf = config.primary_timeframe
    out = {}
    for symbol in config.symbols:
        ...
        if p.exists():
            try:
                ...
                log.warning(f"  Could not reload cached features for {symbol}: {e}")
    return out
...
def run_pipeline(config: Optional[PipelineConfig] = None, args=None) -> Dict:
    ...
    log.info(f"  Symbols ({len(config.symbols)}): {config.symbols}")
    ...
    def _should_run(phase_num: int) -> bool:
        ...
        if _is_phase_done(phase_num, config) and config.cache_datasets:
            return False
        return True
    ...
    if not pipeline_failed and _should_run(3):
        try:
            ...
            if not featured_data:
                ...
                _mark_phase_done(3, config)
        except Exception as e:
            log.error(f"Phase 3 failed: {e}")
            log_phase("phase3_features", "FAILED", error=str(e))
    
    elif not pipeline_failed:
        # Phase 3 skipped via checkpoint -- must reload its cached output into
        # memory or Phase 4/5/6 will see an empty dict and skip too.
        featured_data = _load_cached_features(config)
        if featured_data:
            log.info(f"[SKIP] Phase 3: Feature Engineering (resume or checkpoint) "
                     f"-- reloaded {len(featured_data)} symbols from cache")
        else:
            log.warning("[SKIP] Phase 3: Feature Engineering (resume or checkpoint) "
...
# ── Per-symbol sequential mode ────────────────────────────────────
#
# The default run_pipeline() above processes phase-by-phase across ALL
# symbols at once (Phase 1 downloads all 48, then Phase 2 validates all 48,
# etc.) -- meaning every symbol's data has to be resident at once, which is
# heavy on RAM with a large symbol universe.
#
# run_pipeline_per_symbol() instead runs the ENTIRE pipeline (Phase 1-17)
# for one symbol completely, saves its best_model.json, frees that symbol's
# data, and only then moves to the next symbol. Two direct benefits:
#   1. Only one symbol's data is ever in memory -> far lower peak RAM.
#   2. If the process is killed/crashes partway through, every symbol that
#      already finished keeps its best_model.json. Re-running the same
#      command skips those and picks up exactly where it left off.

def run_pipeline_per_symbol(args=None) -> Dict:
    """Run the full pipeline one symbol at a time. See module docstring
    above for why. A symbol is considered fully complete once its
    data/trained_models/{SYMBOL}/best_model.json exists on disk -- that
    file is only written at the very end of Phase 13 for that symbol."""
    ...
    if args:
        ...
        if args.timesteps:
            base_config.rl_timesteps = args.timesteps
        if args.trials:
            base_config.optuna_trials = args.trials
        if args.no_optuna:
            base_config.optuna_trials = 0
        if args.skip_rl:
            base_config.rl_algorithms = []
        if args.fresh:
            _clear_checkpoints()
            for p in PIPELINE_CACHE_DIR.glob("*_features.parquet"):
                p.unlink()
    ...
    all_symbols = list(base_config.symbols)
    already_done = [s for s in all_symbols if (MODEL_OUTPUT_DIR / s / "best_model.json").exists()]
    todo = [s for s in all_symbols if s not in already_done]

    log.info("=" * 60)
    log.info("  PER-SYMBOL SEQUENTIAL TRAINING MODE")
    log.info(f"  Total symbols: {len(all_symbols)}")
    if already_done:
        log.info(f"  Already completed, skipping ({len(already_done)}): {already_done}")
    log.info(f"  Remaining to process ({len(todo)}): {todo}")
    log.info("=" * 60)

    all_best: Dict[str, Dict] = {}
    failed_symbols = []

    # Pull in results already on disk from a prior (possibly interrupted) run
    for symbol in already_done:
        try:
            all_best[symbol] = json.loads((MODEL_OUTPUT_DIR / symbol / "best_model.json").read_text())
        except Exception as e:
            log.warning(f"  Could not reload existing result for {symbol}: {e}")

    for i, symbol in enumerate(todo, 1):
        log.info("=" * 60)
        log.info(f"  [{i}/{len(todo)}] Starting {symbol} ({len(already_done) + i}/{len(all_symbols)} overall)")
        log.info("=" * 60)
        sym_config = copy.deepcopy(base_config)
        sym_config.symbols = [symbol]
        try:
            result = run_pipeline(config=sym_config)
            if result.get("best_models"):
                all_best.update(result["best_models"])
                log.info(f"  [OK] {symbol} complete and saved -- safe to stop/restart from here.")
            else:
                log.warning(f"  [DONE-NO-MODEL] {symbol} finished but produced no best model "
                            f"(see its phase logs above for why -- e.g. insufficient data).")
        except Exception as e:
            log.error(f"  [FAILED] {symbol}: {e} -- moving on to the next symbol; "
                      f"re-run the same command later to retry just this one.")
            failed_symbols.append(symbol)
            continue

    # Rebuild one combined dashboard covering every symbol processed so far
    try:
        dashboard_path = generate_dashboard({}, all_best, {}, base_config)
        log.info(f"  Combined dashboard: {dashboard_path}")
    except Exception as e:
        log.warning(f"  Could not generate combined dashboard: {e}")

    log.info("=" * 60)
    log.info(f"  PER-SYMBOL RUN COMPLETE — {len(all_best)}/{len(all_symbols)} symbols have a trained model")
    if failed_symbols:
        log.warning(f"  Failed this run ({len(failed_symbols)}): {failed_symbols} "
                    f"-- just re-run the same command, completed symbols will be skipped automatically")
    for symbol, info in sorted(all_best.items()):
        log.info(f"    {symbol}: {info['best_model']} (score={info['score']:.1f})")
    log.info("=" * 60)

    return {"best_models": all_best, "failed_symbols": failed_symbols}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--per-symbol", action="store_true",
                        help="Process one symbol fully (all 17 phases) before moving to the next, "
                             "instead of one phase across all symbols. Lower peak RAM, and safe to "
                             "stop/restart -- completed symbols are skipped on the next run.")
    
    args = parser.parse_args()
    if args.per_symbol:
        run_pipeline_per_symbol(args=args)
    else:
