...
def main() -> int:
    ...
    parser.add_argument(
        "--label-horizon", type=int, default=4,
        help="Bars each label's window looks forward (must match "
             "LabelGenerator.default_horizon, currently 4). Used to purge "
             "train/val rows whose label window overlaps the next split — "
             "see ml/cv_splitter.py. Pass 0 to explicitly opt out (NOT "
             "recommended for models that will feed live ensemble decisions).",
    )
    parser.add_argument(
        "--no-purge", action="store_true",
        help="Disable purged/embargoed splitting (reproduces the old leaky "
             "default). Only use this for a controlled A/B comparison against "
             "the purged pipeline — never for models you intend to trade live.",
    )
    args = parser.parse_args()

    from ml.model_trainer import train_all

    use_purged_split = not args.no_purge
    label_horizon = 0 if args.no_purge else args.label_horizon

    print("=" * 60)
    print("  Retraining base models")
    if use_purged_split:
        print(f"  (purged/embargoed split: label_horizon={label_horizon} bars)")
    else:
        print("  (WARNING: --no-purge set - naive split, label-window leakage possible)")
    print("=" * 60)
    results = train_all(
        pair=args.pair, timeframe=args.timeframe, min_samples=args.min_samples,
        use_purged_split=use_purged_split, label_horizon=label_horizon,
    )
