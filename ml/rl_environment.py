...
class ForexTradingEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        # Reset the reward engine's per-episode state (e.g. the capped
        # consecutive-idle-steps counter). The engine is usually a shared
        # singleton across episodes, so this must happen here.
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # ── Bankruptcy / margin-call termination ────────────────────
        # ENHANCEMENT: without this, a single early bad trade that blows
        # the account can leave the agent stuck at deeply negative
        # balance for the REST of the episode (thousands of steps), each
        # one hitting the reward engine's drawdown-penalty floor
        # (-20/step). That drowns the learning signal in floor-clipped
        # noise and wastes rollout time on an episode that's already
        # decided. A real broker would margin-call the account long
        # before this; the environment should too.
        bankrupt = self.balance <= self.initial_balance * 0.2  # 80% drawdown
        if bankrupt and self.position.direction != "NONE":
            self._close_position(reason="margin_call")
