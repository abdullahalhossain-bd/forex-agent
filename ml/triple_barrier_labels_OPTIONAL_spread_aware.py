...
def triple_barrier_labels(
    ...
    *,
    ...
    spread_col : name of the column holding broker spread in POINTS (e.g.
        MT5's `spread` field). When given, barriers are floored at
        `min_cost_multiple x round_trip_spread` so a bar can never be
        labeled a 'win' on a move that's smaller than what it costs to
        trade. Wide-spread symbols (GBPNOK/GBPSEK-type crosses, where
        spread routinely runs 60-90% of a 1.5xATR target) were showing
        80%+ raw win rate that back-tested to roughly break-even or
        negative once the spread was actually paid — this parameter is
        the fix for that specific failure mode (see engineering report).
    point_size : price-per-point for this symbol (0.00001 for 5-digit
        quotes, i.e. the exact same "spread" units MT5 reports).
    min_cost_multiple : minimum barrier width in *round-trip spread*
        units. 3.0 is a conservative floor: it forces the labeled TP/SL
        distance to be worth at least 3x what it costs to enter, roughly
        offsetting the unmodeled real-world slippage on top of spread.
    ...
    if spread_col is not None and spread_col in df.columns:
        cost_price = df[spread_col].astype(float) * point_size
        min_barrier = min_cost_multiple * cost_price
    else:
        min_barrier = pd.Series(0.0, index=df.index)
    ...
    for i in range(n - holding_period):
        ...
        tp_dist = max(take_profit_width * atr_val, min_barrier.iloc[i])
        sl_dist = max(stop_loss_width * atr_val, min_barrier.iloc[i])
        upper = entry_price + tp_dist
