...
class RewardEngine:
    ...
    def __init__(
        ...
        max_idle_steps_rewarded: int = 20,
    ):
        ...
        # BUG FIX (reward hacking): hold_reward used to fire unconditionally
        # on every flat HOLD step. Over a 5,000-step episode that is
        # 0.1 * 5000 = 500 in guaranteed cumulative reward for NEVER
        # trading — far larger than anything a real trade can earn once
        # rewards are clipped to [-20, +20] per step. PPO learns the
        # optimal policy is "stay flat forever" (a degenerate, non-trading
        # agent that still looks "profitable" in the training logs).
        # Fix: only reward patience for a bounded number of CONSECUTIVE
        # idle steps (reset the moment a position opens), so the maximum
        # total idle reward any stretch of flat time can contribute is
        # capped at `hold_reward * max_idle_steps_rewarded`, and can never
        # outgrow the reward available from actually trading.
        self.max_idle_steps_rewarded = max_idle_steps_rewarded
        self._consecutive_idle_steps = 0

    def reset_episode(self) -> None:
        """Reset per-episode state. The reward engine is typically a
        singleton shared across episodes (`get_reward_engine()`), so the
        environment MUST call this in `reset()` — otherwise the idle-step
        counter would carry over from the previous episode and either
        under- or over-reward the first steps of the new one."""
        self._consecutive_idle_steps = 0
    ...
    def calculate(
        ...
        # Bounded: only the first `max_idle_steps_rewarded` CONSECUTIVE
        # idle steps earn a reward. Any action other than a flat HOLD
        # resets the counter, so an agent can't accumulate unbounded
        # reward simply by sitting out the whole episode (see fix note
        # in __init__ — this was the root cause of a "never trades"
        # degenerate policy).
        if action == "HOLD" and not position_open:
            ...
            if self._consecutive_idle_steps <= self.max_idle_steps_rewarded:
                ...
                rb.reason = "idle (patience reward already capped this stretch)"
        else:
            self._consecutive_idle_steps = 0
