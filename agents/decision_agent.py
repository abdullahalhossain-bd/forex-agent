...
class DecisionAgent:
    ...
    # FIX: configurable via env so the operator can tune the floor
    # without code changes. Logs showed signals at 60-66% confidence
    ...
    #
    # FIX (2026-07-25): operator feedback after production-log review:
    # the 85% floor was too high — production logs showed ZERO trades
    # being placed in entire sessions because no single layer reached
    # 85% confidence. Operator requested 70% as a more practical floor
    ...
    def decide(
        ...
        # ── EXECUTION-PROOF AUDIT FIX: zone_score confidence modifier ──
        # The CONSUMPTION-MAP audit proved `unified_signal.zone_score`
        # (computed by OddEnhancerScorer at analysis_agent.py:2186-2214)
        # was DEAD — no downstream code read it. The original comment at
        # analysis_agent.py:2185 said "a zone_score that the decision
        # system can use" — that wiring was never done. Now done: a high
        # tier zone adds up to +5% confidence, a low tier subtracts up
        # to -5%. This is a soft modifier (not a hard gate) because zone
        # quality is one input among many — but it MUST influence the
        # final number, otherwise the OddEnhancers computation is
        # wasted CPU every cycle.
        try:
            _unified = analysis_out.get("unified_signal") if isinstance(analysis_out, dict) else None
            if isinstance(_unified, dict):
                _zs = _unified.get("zone_score")
                if isinstance(_zs, dict):
                    _tier = (_zs.get("tier") or "").upper()
                    _total = float(_zs.get("total_score", 0) or 0)
                    _max = float(_zs.get("max_score", 1) or 1)
                    _ratio = (_total / _max) if _max > 0 else 0.0
                    # Tier-based modifier: A=+5, B=+2, C=-2, D=-5
                    _tier_mod = {"A": 5, "B": 2, "C": -2, "D": -5}.get(_tier, 0)
                    # Proportional modifier: ±2% based on score ratio
                    _ratio_mod = int((_ratio - 0.5) * 4) if 0 <= _ratio <= 1 else 0
                    _zone_mod = max(-7, min(7, _tier_mod + _ratio_mod))
                    if _zone_mod != 0:
                        adj_conf = max(0, min(99, adj_conf + _zone_mod))
                        log.info(
                            f"[DecisionAgent] Zone score modifier: {_zone_mod:+d}% "
                            f"(tier={_tier}, score={_total:.0f}/{_max:.0f})"
                        )
        except Exception as _e_zs:
            log.debug(f"[DecisionAgent] Zone score modifier failed: {_e_zs}")
