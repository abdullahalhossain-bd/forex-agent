...
class RiskEngine:
    ...
    # Broker-imposed lot floor — config-driven (config.MIN_LOT). Was
    # hardcoded to 0.01 inline at the lot-clamp call site below; now a
    # class attribute so it's visible alongside MAX_LOT and can be
    # compared against it (see the MIN_LOT>=MAX_LOT bypass further down).
    from config import MIN_LOT as _CFG_MIN_LOT
    ...
    def evaluate(self, signal: str, entry: float, atr: float, regime: dict | None = None,
        ...
        # Floor is self.MIN_LOT (config-driven), was hardcoded 0.01.
        ...
        if lot_raw > self.MAX_LOT:
            ...
            #
            # BYPASS (2026-09-10 fix): when self.MIN_LOT >= self.MAX_LOT,
            # position size is mechanically pinned to a single broker-floor
            # lot (e.g. a small Cent-account balance with MIN_LOT==MAX_LOT
            # ==0.01) — there is no larger lot to size up to, so this
            # fraction can NEVER be satisfied for any trade whose target-
            # risk lot exceeds that floor. Without this bypass, the guard
            # rejected essentially every signal on such accounts (see
            # 2026-09-10 log: EURUSD/GBPUSD rejected at 16-18% of intended
            # despite MAX_LOT already matching MIN_LOT — not a real safety
            # finding, just an artifact of zero sizing headroom). The floor
            # lot's own risk is still separately capped at MAX_RISK_PCT by
            # the pre-floor gate below (lot_raw < self.MIN_LOT branch), so
            # bypassing this specific guard can only ever result in LESS
            # risk than intended, never more.
            if self.MIN_LOT >= self.MAX_LOT:
                log.info(
                    f"[RiskEngine] {self.symbol}: MIN_LOT ({self.MIN_LOT}) >= "
                    f"MAX_LOT ({self.MAX_LOT}) — position size is pinned to "
                    f"the broker floor with no sizing headroom. Skipping the "
                    f"MIN_RISK_FRACTION_OF_INTENDED guard (would reject "
                    f"nearly every trade on this account by construction); "
                    f"trading at {risk_pc:.4f}% (${risk_usd}) instead of the "
                    f"{risk_pc_intended:.2f}% target — under-risked, not "
                    f"over-risked."
                )
            else:
                try:
        ...
        # Pre-floor safety gate (NEW, 2026-09-10): the code above only
        # handled lot_raw > MAX_LOT (cap DOWN, which can only reduce risk
        # below intended — safe). It never handled the opposite case,
        # lot_raw < MIN_LOT: `lot` was still floored UP to self.MIN_LOT by
        # the round(max(self.MIN_LOT, ...)) call above, but risk_pc/risk_usd
        # were left equal to the PRE-floor intended values — so on a small
        # account (e.g. this Cent account, MIN_LOT=0.01) where the target-
        # risk lot for a wide-SL trade is smaller than the broker's own
        # minimum, the reported "risk_pc" understated the REAL risk the
        # floored MIN_LOT actually took, and a trade that genuinely
        # breached MAX_RISK_PCT at MIN_LOT could be silently approved.
        # Recompute from the actual floored `lot` and reject if it breaches
        # the hard cap — mirrors the equivalent gate already present in
        # agents/risk_agent.py and risk/position_sizer.py, now enforced
        # here too since RiskEngine (this class) is what core/trader.py
        # actually uses for live trades.
        if lot_raw < self.MIN_LOT:
            floor_risk_usd = round(self.MIN_LOT * sl_pips * pip_val, 2)
            floor_risk_pc  = round((floor_risk_usd / self.balance) * 100, 4) if self.balance > 0 else float("inf")
            try:
                from config import MAX_RISK_PCT as _CFG_MAX_RISK_PCT
                _max_risk_pct = float(_CFG_MAX_RISK_PCT) * 100  # config stores a fraction; compare in percent
            except Exception:
                _max_risk_pct = self.MAX_RISK_PC
            if floor_risk_pc > _max_risk_pct:
                return self._reject(
                    f"Balance too small for {self.symbol}: even the broker "
                    f"minimum lot ({self.MIN_LOT}) risks {floor_risk_pc:.4f}% "
                    f"(${floor_risk_usd}) of the ${self.balance:.2f} balance, "
                    f"above the {_max_risk_pct:.2f}% hard cap "
                    f"(sl_pips={sl_pips}, lot_intended={lot_raw:.4f})"
                )
            log.info(
                f"[RiskEngine] {self.symbol}: intended lot {lot_raw:.4f} "
                f"floored up to broker minimum {self.MIN_LOT} — actual risk "
                f"{floor_risk_pc:.4f}% (${floor_risk_usd}) vs "
                f"{risk_pc_intended:.2f}% target (within the "
                f"{_max_risk_pct:.2f}% hard cap)."
            )
            risk_usd = floor_risk_usd
            risk_pc  = floor_risk_pc
