...
class ConfidenceEngine:
    ...
    def calculate(
        ...
        # ── Round-7 audit fix: use base_confidence as prior when no data ──
        # Previously: when sample_size==0, all three component scores
        # returned flat 50.0 → raw_final = 50×0.5 + 50×0.3 + 50×0.2 = 50
        # → bayesian_penalty −8 → final = 42% → below 55% threshold →
        # NO TRADE forever (chicken-and-egg loop).
        #
        # The `base_confidence` parameter (rule 91% / LLM 92%) was
        # COMPLETELY IGNORED in the formula — only used to compute the
        # cosmetic `adjustment` field at the end. The operator's audit
        # caught this: "base_confidence parameter একদমই formula তে
        # ব্যবহার হয় না".
        #
        # Now: when historical data is missing, use base_confidence as
        # the prior for the historical_score component. This makes
        # raw_final ≈ base_confidence on a fresh account (e.g. 91%),
        # so after −8 bayesian penalty the result is 83% — well above
        # the 55% tier-3 threshold.
        if sample_size == 0:
            historical_score = float(base_confidence)
        ...
        # AND no recent_results AND no regime_stats), the formula still
        # weights them 50/50/50 → 50. The historical_score override above
        # fixes the largest weight (50%), but recent_score and regime_score
        # still pull toward 50. To make base_confidence properly act as
        # the prior on a fresh account, we blend toward base_confidence
        # when sample_size is below the minimum threshold.
        if sample_size < 3:  # below MIN_SAMPLE_SIZE
            # Soft blend: pull recent_score and regime_score toward
            # base_confidence proportionally to how little data we have.
            # At sample_size=0: full blend (100% toward base_confidence)
            # At sample_size=1: 67% blend
            # At sample_size=2: 33% blend
            # At sample_size>=3: no blend (use raw scores)
            blend_ratio = max(0.0, 1.0 - sample_size / 3.0)
            recent_score = (
                recent_score * (1.0 - blend_ratio)
                + float(base_confidence) * blend_ratio
            )
            regime_score = (
                regime_score * (1.0 - blend_ratio)
                + float(base_confidence) * blend_ratio
            )
    ...
    def _bayesian_penalty(self, sample_size: int) -> float:
        ...
        # Round-12: check if we're in system-wide bootstrap mode
        # (no pattern has accumulated enough trades yet)
    ...
    def _is_system_bootstrap(self) -> bool:
        """Round-12: check if the ENTIRE system has <3 closed trades.

        In bootstrap mode (system-wide 0-2 closed trades), Bayesian
        penalties are reduced so high-confidence signals can clear
        the Tier-1 80% threshold and actually execute — breaking the
        chicken-and-egg loop where no trades = no data = no trades.

        Once ANY pattern accumulates >=3 trades, the system has
        "proven" it can trade, and normal Bayesian penalties resume.
        """
        try:
            stats = self._load_stats()
            if not stats:
                return True  # no stats file = definitely bootstrap
            # Check if ANY pattern has >=3 trades
            for key, entry in stats.items():
                if entry.get("total_trades", 0) >= MIN_SAMPLE_SIZE:
                    return False  # at least one pattern has data
            return True  # no pattern has enough trades
        except Exception:
            return True  # fail-safe: assume bootstrap
