...
def run_live_mirror_engine(*, symbol: str, df: pd.DataFrame, timeframe: str = "H1",
    ...
    """Replay the shared decision kernel with a real historical execution boundary."""
    ...
    router = HistoricalExecutionRouter(broker, max_open_positions=max_open_trades,
    ...
    for i in range(int(warmup_bars), len(df)):
        stats["total_bars"] += 1
        ts = df.index[i]
        replay_clock.set(ts) if hasattr(replay_clock, "set") else None

        # 1. Resolve exits from already-open positions using only this bar.
        still_open = []
        for trade in open_trades:
            opened = entry_bar[trade.trade_id]
            result = broker.check_exit(trade, float(df.iloc[i]["high"]),
                                       float(df.iloc[i]["low"]), float(df.iloc[i]["close"]), ts)
            if result is not None:
                result.hold_bars = i - opened
                closed_trades.append(result)
                router.on_close(trade, reason=result.exit_reason)
            elif i - opened > max_hold_bars:
                result = broker.close_trade(trade, float(df.iloc[i]["close"]), ts, "timeout")
                result.hold_bars = i - opened
                closed_trades.append(result)
                router.on_close(trade, reason="timeout")
            else:
                still_open.append(trade)
        open_trades = still_open

        # 2. Fill orders submitted on prior bars / pending limits.
        ...
        # 3. Shared live decision kernel — no alternate strategy implementation.
        try:
        ...
        # No synthetic ATR fallback: risk levels must come from the canonical risk path.
        sl = risk_out.get("sl_price")
        tp = risk_out.get("tp_price")
        lot = risk_out.get("lot")
        ...
        if requested_entry is None:
            requested_entry = float(df.iloc[i]["close"])

        result = router.submit(symbol=symbol, direction=action, entry_price=float(requested_entry),
                               sl=float(sl), tp=float(tp), lot=float(lot),
                               confidence=int(dec_out.get("confidence", 0) or 0),
    ...
    # Pending requests are never silently converted into fills at replay end.
    ...
    for trade in list(open_trades):
        ...
        closed_trades.append(result)
        router.on_close(trade, reason="end_of_backtest")

    metrics = calculate_metrics(trades=closed_trades, starting_balance=starting_balance,
                                ending_balance=broker.get_balance())
    return MirrorRunResult(symbol=symbol, timeframe=timeframe, bars=len(df),
                           trades=closed_trades, equity_curve=equity,
                           rejection_stats=stats, metrics=metrics)
