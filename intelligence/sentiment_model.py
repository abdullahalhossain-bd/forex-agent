        ...
        if raw is None:
            ...
            if gemini_client is not None:
                try:
                    ...
                    # Demoted from WARNING to DEBUG: a 401/403 here is
                    # already reported by the LLM KeyManager (with
                    # cooldown/permanent-disable bookkeeping). Logging
                    # the same failure AGAIN at the sentiment layer
                    # produced one extra noisy line per dead key per
                    # cycle (16+ keys × N cycles). The rule-based
                    # fallback in _analyze_with_rules() still kicks in
                    # automatically; operators should look at the
                    # KeyManager ERROR lines, not here.
                    log.debug(f"[SentimentModel] Gemini failed: {e}")

        # ── 2026-07-25 NEW: OpenRouter as FALLBACK #2 ──────────────
        ...
        if raw is None and _key_manager is not None and _key_manager.has_any_openrouter:
            try:
                ...
                if or_client is not None:
                    or_model = os.getenv("OPENROUTER_MODEL", "google/gemma-4-26b-a4b-it:free")
                    ...
                    log.debug(f"[SentimentModel] OpenRouter fallback success | {or_model}")
            except Exception as e:
                if _key_manager is not None:
                    is_rate_limited = "429" in str(e) or "rate limit" in str(e).lower()
