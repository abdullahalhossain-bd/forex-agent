#!/usr/bin/env python3
"""
Comprehensive Winrate & Profit Test
=====================================
Runs the production SignalEngine on all 7 pairs with historical data
to measure winrate and profit with the v3 improvements applied.
"""
...
def detect_pattern(df, i):
    ...
    if bp < 0.3 and (row['low'] - min(row['open'], row['close'])) > 2 * body:
        return {'pattern_signal': 'Bullish', 'latest_pattern': 'hammer'}
    if bp < 0.3 and (row['high'] - max(row['open'], row['close'])) > 2 * body:
...
def run_backtest(symbol, min_confidence=70):
...
def main():
    print("=" * 80)
    print("  COMPREHENSIVE WINRATE & PROFIT TEST")
    print("  V3 Production Code with All Improvements")
    print("=" * 80)
    ...
    for symbol in symbols:
        print(f"\n{'='*60}")
        print(f"  Testing {symbol} H1...")
        print(f"{'='*60}")

        result = run_backtest(symbol, min_confidence=70)
        if result is None:
        ...
        print(f"  Return: {(final_balance/result['initial_balance']-1)*100:.2f}%")

        if len(buy_trades) > 0:
            print(f"  BUY: {len(buy_trades)} trades, WR={buy_wins/len(buy_trades)*100:.1f}%")
        if len(sell_trades) > 0:
            print(f"  SELL: {len(sell_trades)} trades, WR={sell_wins/len(sell_trades)*100:.1f}%")
        ...
        all_results.append({
            ...
            'return_pct': round((final_balance/result['initial_balance']-1)*100, 2),
    ...
    if all_results:
        ...
        print(f"\n  Total trades across all pairs: {total_trades}")
        ...
        print(f"\n  Per-Pair Results:")
        print(f"  {'Symbol':<10} {'Trades':>7} {'WR':>7} {'PF':>6} {'PnL':>10} {'Return':>8}")
        print(f"  {'-'*50}")
        for _, row in results_df.iterrows():
            print(f"  {row['symbol']:<10} {row['total_trades']:>7} {row['win_rate']:>6.1f}% {row['profit_factor']:>6.2f} ${row['total_pnl_usd']:>9.2f} {row['return_pct']:>7.1f}%")

        # Save results
        results_df.to_csv(OUTPUT / 'winrate_profit_results.csv', index=False)
        trades_df = pd.DataFrame(all_trades)
        trades_df.to_csv(OUTPUT / 'winrate_profit_trades.csv', index=False)
        print(f"\n  Results saved to:")
        print(f"    {OUTPUT / 'winrate_profit_results.csv'}")
        print(f"    {OUTPUT / 'winrate_profit_trades.csv'}")
