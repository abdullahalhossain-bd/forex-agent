...
class NewsFilter:
    ...
    def _fetch_events(self) -> tuple[list, str]:
        ...
        # Demoted from WARNING to DEBUG: the fallback is the *expected*
        # path whenever Forex Factory's anti-bot wall blocks the scraper
        # (which is most of the time in 2026). The 5-minute cooldown
        # above ensures we don't keep hammering the wall; this log line
        # was firing every cycle and drowning genuinely actionable
        # warnings in noise. The "fallback active" status is still
        # visible in the returned `source` field.
        log.debug("Live Forex Factory fetch fully failed → hardcoded fallback")
    ...
    def _fetch_html_cloudscraper(self) -> str | None:
        try:
            if self._scraper is None:
                # `delay=10` makes cloudscraper wait 10s on first request
                # to compute the Cloudflare clearance token — this improves
                # the success rate vs. the default immediate retry which
                # often 403s because the token isn't ready yet.
            ...
            # Demoted from WARNING to DEBUG: a 403 from forexfactory.com
            # is the *expected* outcome — their Cloudflare wall blocks
            # every scraper we've tried (cloudscraper, requests, etc.).
            # WARNING-level was producing ~3 log lines per cycle for a
            # condition that always falls through cleanly to the
            # hardcoded fallback (Day 101+ real 2026 release calendar).
            log.debug(f"cloudscraper fetch failed (expected — FF anti-bot wall): {e}")
            return None
    ...
    def _fetch_html_requests_with_retry(self) -> str | None:
        ...
        for attempt in range(1, self.FETCH_MAX_RETRIES + 1):
            try:
                ...
                last_err = e
                # Demoted to DEBUG — same reasoning as the cloudscraper
                # log above. Plain requests against forexfactory.com
                # almost always 403s; the cooldown + hardcoded fallback
                # handles this cleanly.
                log.debug(
                    f"Forex Factory plain-requests fetch failed "
                    f"(attempt {attempt}/{self.FETCH_MAX_RETRIES}, expected — anti-bot wall): {e}"
                )
                if attempt < self.FETCH_MAX_RETRIES:
                    time.sleep(self.FETCH_RETRY_DELAY_S * attempt)
        log.debug(f"Forex Factory fetch exhausted retries — last error: {last_err}")
        return None
