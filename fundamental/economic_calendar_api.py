...
class EconomicCalendarAPI:
    ...
    def get_calendar(
        ...
        # Phase 2.5 (backtest infra optimization): a historical bar has no
        # "next 24h" of real economic events to fetch — skip the live
        # calendar API call entirely and return neutral (no block).
        from core.constants import is_backtest_mode
        if is_backtest_mode():
            return self._empty_result("backtest mode — live calendar skipped", block=False)
        ...
        if not events:
            # ── FIX: previously a calendar outage = total trade block.
            # In production logs (trader.log), this fired 5+ times back-to-
            # back, blocking every trade for the entire session even though
            # the calendar outage was a transient network issue (no high-
            # impact event was actually scheduled, but we couldn't prove it).
            #
            # New behavior: if all sources fail AND there's no fallback
            # signal we can trust, downgrade from "block all trades" to
            # "block only high-risk trades" — i.e. block=True but only for
            # the next 30 minutes (effectively: "we don't know, so be
            # conservative but don't kill the whole session"). Caller code
            # that checks trade_block will still skip, but a separate flag
            # `calendar_outage` lets upstream code decide whether to apply
            # the block strictly or treat it as advisory.
            log.warning(
                "[EconCal] All calendar sources returned 0 events — "
                "marking calendar_outage=True. trade_block is set to True "
                "for safety, but downstream code can override via env var "
                "ECONCAL_OUTAGE_ALLOWS_TRADES=true if calendar reliability "
                "is known to be poor."
            )
    ...
    @staticmethod
    def _empty_result(reason: str, block: bool = True,
                      calendar_outage: bool = False) -> Dict[str, Any]:
        """Return a safe empty result.

        block defaults to True — calendar outage = unknown risk = no trading.
        calendar_outage flag distinguishes "no events scheduled" (legitimate
        empty calendar) from "couldn't fetch calendar" (network failure).
        Downstream code can use this flag to override the block when the
        calendar source is known to be unreliable.
        """
        return {
            "source":            "none",
            "events":            [],
