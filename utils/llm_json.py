...
def parse_llm_json(raw: Optional[str]) -> Any:
    ...
    start = text.find("{")
    if start < 0:
        # ── FIX: many LLM "no JSON" failures we saw in trader.log had
        # responses like "We need to produce JSON with fields: market_story,
        # key_levels (array of three floats), trade_plan (object with signal,
        # e..." — the LLM was confused and echoing back the prompt's
        # field-spec as prose instead of producing JSON. In these cases the
        # LLM clearly understood the schema; it just didn't emit JSON.
        # Try a final recovery: look for a colon-prefixed JSON-shaped block
        # and reconstruct. If nothing works, raise with a clearer message
        # so the master_analyst's retry loop knows to send a stronger prompt.
        log.warning(
            f"[llm_json] no JSON object in response (len={len(text)}). "
            f"The LLM may have echoed the prompt's field-spec instead of "
            f"producing JSON. Caller should retry with a more explicit "
            f"instruction. Preview: {text[:200]!r}"
        )
        raise json.JSONDecodeError(
            "No JSON object ('{') found in LLM response — LLM likely "
            "echoed the prompt spec instead of producing JSON. Retry.",
            text, 0
        )
