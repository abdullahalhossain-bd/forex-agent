...
# ── Shared handler cache ────────────────────────────────────────
# Bugfix: get_logger() previously created a BRAND NEW RotatingFileHandler
# (and console StreamHandler) every time it was called with a NEW logger
# name — e.g. get_logger("strategy_selector"), get_logger("news_filter"),
# get_logger("analysis_agent") each opened their OWN separate file handle
# to the SAME logs/trader.log path. A single process ended up holding
# many open handles to one file.
#
# On Windows, a RotatingFileHandler rotates by renaming the file
# (os.rename). Windows refuses to rename a file that has ANY open handle
# on it — including handles held by the SAME process. So the moment one
# handler tried to rotate, every other handler's still-open handle on
# trader.log blocked the rename with PermissionError: [WinError 32].
#
# Fix: build the file handler and console handler ONCE at module import
# time and have every logger share those same two handler instances.
# One handler = one open file handle = rotation can actually rename it.
