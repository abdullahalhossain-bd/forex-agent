...
class LiquidityZoneMapper:
    ...
    IMPORTANT (no-lookahead contract): every method here only reads swing
    points that are already confirmed (a swing needs SWING_WINDOW closed
    candles *after* it to qualify — see the `range(w, n - w)` loops), and
    PDH/PDL/PWH/PWL always use unique_days/weeks[-2], i.e. the last fully
    completed period, never the in-progress one. That makes it safe to call
    on an ever-growing `df`. It is NOT safe to call once on a full historical
    dataframe and then treat the output as if it were "known" at some earlier
    bar — for backtesting, call these methods on `df.iloc[:i+1]` at each bar
    `i` (expanding window), not on the whole dataset. Also ensure `df`'s last
    row is a fully CLOSED candle, never the currently-forming one.
    """
    ...
    def _cluster_points(self, points: list[tuple[int, float]], tolerance: float) -> list[list[tuple[int, float]]]:
        ...
        BUGFIX: was comparing each new point to the *running mean* of the
        cluster. Since the mean drifts as points are added, a chain of small
        sequential increments (each individually within tolerance of the
        current mean) could accumulate into a cluster whose first and last
        points are much further apart than `tolerance` — e.g. four touches
        spaced 0.0003 apart with tolerance=0.0005 would all merge into one
        "equal high" despite spanning 0.0009. That silently mislabels a
        sloping run of highs as a flat liquidity pool. Fixed to compare each
        candidate against the cluster's anchor (first) point, which bounds
        total cluster width to `tolerance`.
        """
        if not points:
            return []
        ...
        for pt in sorted_pts[1:]:
            anchor_price = clusters[-1][0][1]
            if abs(pt[1] - anchor_price) <= tolerance:
