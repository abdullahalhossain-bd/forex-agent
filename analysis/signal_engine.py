...
class SignalEngine:
    ...
    def generate(
        ...
        timestamp:        object = None,   # CLAUDE FIX: see session gate below
    ) -> dict:
        """
        ...
        CLAUDE FIX (see backtest report): added optional `timestamp` param.
        When provided, applies a session-aware confidence floor sourced from
        analysis/session_rules.py's SESSION_STRATEGIES (via SessionAnalyzer).
        That table already labeled LONDON_NY_OVERLAP "A_PLUS_ONLY" but never
        actually enforced a stricter min_confidence there than any other
        session -- and this generate() method never consulted session data
        at all. Walk-forward backtest evidence (~700 PRIMARY trades) found
        LONDON_NY_OVERLAP + NEW_YORK accounted for -74.5R of -75.2R total
        loss. session_rules.py has been patched (min_confidence 60 -> 85 for
        those two sessions); this wires that value in as an additional
        confidence floor, soft (per-trade), not a session-wide hard block.
        Fully backward compatible: if timestamp is None (no caller passes
        it), behavior is 100% unchanged from before this fix.
        """
        signals  = []
        warnings = []
        bull_score = 0
        ...
        # CLAUDE FIX: session-aware confidence floor (soft downgrade, not a
        # session-wide block). See docstring above for the evidence.
        if timestamp is not None and signal in ('BUY', 'STRONG_BUY', 'SELL', 'STRONG_SELL'):
            try:
                from analysis.session_analyzer import SessionAnalyzer
                _sess = SessionAnalyzer()
                _sess_info = _sess.get_current_session(timestamp)
                _sess_name = _sess_info.get('primary_session') if isinstance(_sess_info, dict) else None
                if _sess_name:
                    _mode = _sess.get_strategy_mode(_sess_name)
                    _required = _mode.get('min_confidence')
                    if _required is not None and confidence < _required:
                        warnings.append(
                            f"⚠️  {signal} downgraded to WAIT — confidence {confidence} "
                            f"below {_sess_name} session floor ({_required})"
                        )
                        signal = 'WAIT'
                        recommendation = self._signal_recommendation(signal, confidence, warnings)
            except Exception:
                pass  # fail-open: never let session lookup break signal generation
