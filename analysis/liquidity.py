...
# BUGFIX (institutional review): this logger was previously named
# "liquidity_engine", identical to the logger in analysis/liquidity_engine.py
# (a different module with a same-named LiquidityEngine class — see the audit
# report). That made it impossible to tell, from the log stream alone, which
# of the two modules emitted a given line. Renamed to "liquidity" to match
# this file.
#
# BUGFIX 2 (this pass): the class itself was ALSO named `LiquidityEngine`,
# identical to analysis.liquidity_engine.LiquidityEngine — two different
# classes (different constructor, different analyze() signature, different
# return schema) sharing one name. Depending on import order
# (`from analysis.liquidity import LiquidityEngine` vs
# `from analysis.liquidity_engine import LiquidityEngine`), whichever import
# ran last silently wins in any module that does `import *` or re-exports —
# a live wrong-strategy-running risk, not just a style issue. Renamed this
# (Day-61, simpler pools/premium-discount) class to `LiquidityPoolAnalyzer`.
# analysis.liquidity_engine.LiquidityEngine (Day-62, the full pipeline with
# stop-hunt + session confluence + scoring) keeps the LiquidityEngine name
# since it's the actual entry point referenced elsewhere.
# Update any external call sites: LiquidityEngine() -> LiquidityPoolAnalyzer().
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        engine = LiquidityPoolAnalyzer(swing_window=5)
        result = engine.analyze(df)
