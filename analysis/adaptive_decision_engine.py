...
# UPDATED (20-strategy backtest audit): raised from 0.35 -> 0.40. Anything
# below 40% WR is screened out entirely in AdaptiveDecisionEngine.decide()
# (see the "Screen out strategies with a PROVEN poor edge" block below),
# regardless of how many of them agree with each other.
MIN_WIN_RATE_CONF = 0.40

# Default weights (overridden by calibrated backtest results via
# load_backtest_results(), which multiplies these by win_rate/0.5).
#
# UPDATED (20-strategy backtest audit — 5 passed the 40% WR screen):
#   stop_hunt              67.2% WR  -> highest base weight
#   ict_amd                58.6% WR
#   liquidity               42.6% WR
#   cci_state                42.3% WR
#   candlestick_patterns     41.8% WR
#
# The 11 strategies below 40% WR are set to weight=0.0 (excluded from
# scoring even if they somehow bypass the MIN_WIN_RATE_CONF gate above —
# belt-and-suspenders, since both mechanisms should never disagree).
DEFAULT_STRATEGY_WEIGHTS = {
    # ✅ Passed 40% WR screen — ranked by backtested win rate
    "stop_hunt":             3.0,   # 67.2% WR — best performer, top weight
    "ict_amd":               2.5,   # 58.6% WR
    "liquidity":             1.8,   # 42.6% WR
    "cci_state":             1.5,   # 42.3% WR
    "candlestick_patterns":  1.2,   # 41.8% WR

    # ❌ Below 40% WR — excluded (weight=0)
    "crossover":             0.0,   # 38.0% WR
    "fibonacci":             0.0,   # 35.9% WR
    "sr_zones":              0.0,   # 34.5% WR
    "smc_advanced":          0.0,   # 33.0% WR
    "order_block":           0.0,   # 32.1% WR
    "ichimoku":              0.0,   # 30.6% WR
    "market_structure":      0.0,   # ~30.5% WR
    "supply_demand":         0.0,   # 30.3% WR
    "trendline":             0.0,   # 30.2% WR
    "volume_profile":        0.0,   # 29.7% WR
    "smart_money":           0.0,   # ~25% WR

    # Not part of this 16-strategy audit — kept at legacy default,
    # unaffected until backtested against the same standard.
    "pin_bar":               1.5,
    "sd_zones_scored":       2.0,
    "multi_pa":              1.5,
}
