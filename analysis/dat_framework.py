...
class DATFramework:
    ...
    AREA STAGE STATUS (institutional review, item #2 — updated 2026-07):
        ...
        + FVG + Fibonacci" but only SupportResistance was actually wired
        in. That gap is now closed: FVG (analysis/fvg_detector.py),
        Fibonacci (analysis/fibonacci.py), and OrderBlock
        (analysis/order_block.py) are all imported and consulted by
        _evaluate_area(). `area_zone_type` can now genuinely be any of
        {"support", "resistance", "fvg", "fibonacci", "order_block"}.
    """
    ...
    def __init__(self):
        ...
        # OrderBlock (analysis/order_block.py) — imported alongside
        # FVG/Fibonacci above. Has its own try/except so a failure here
        # doesn't silently disable the other area checks (see item #2).
        try:
