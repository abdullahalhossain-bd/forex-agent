...
class NewsAPIProvider:
    ...
    def fetch_headlines_for_pair(self, pair: str) -> Dict[str, Any]:
        ...
        # Phase 2.5 (backtest infra optimization): skip NewsAPI AND the
        # RSS fallback (intelligence/news_sources.py — also a live network
        # call) during historical replay. Today's headlines have no
        # relevance to a historical bar's timestamp.
        from core.constants import is_backtest_mode
        if is_backtest_mode():
            return self._fallback_result("backtest mode — live news skipped")
