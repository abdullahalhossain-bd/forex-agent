...
class SentimentEngine:
    ...
    def fear_greed(self, index_value: float, source: str = "fx_native") -> dict:
        """
        Fear & Greed Index দেখে market emotion বোঝো।

        Args:
            index_value : 0 (Extreme Fear) → 100 (Extreme Greed)
            source      : where index_value came from. The default is now
                "fx_native" — the FX-native aggregator in
                sentiment_data.py:get_fear_greed_index() that averages
                retail long% across 7 major pairs (OANDA → Myfxbook →
                synthetic RSI). The crypto F&G index from alternative.me
                is only used as a last-resort fallback and is flagged
                source="alternative.me".

        Source-aware scoring (round-5 audit fix, Finding H-3):
            - "fx_native"      → full-weight score (genuine FX sentiment)
            - "alternative.me" → half-weight score (crypto cross-asset proxy;
                                 no validated crypto↔FX correlation exists)
            - "fallback"       → half-weight score (no real data; same
                                 low-confidence penalty as crypto proxy)
        ...
        # ── Round-5 audit fix: source-aware scoring for F&G ──
        #
        # sentiment_data.py:get_fear_greed_index() now returns one of three
        # sources, in priority order:
        #   1. "fx_native"      — FX-native aggregator (retail long% across
        #                         7 major pairs via OANDA/Myfxbook). Genuine
        #                         FX-market sentiment; scored at full weight.
        #   2. "alternative.me" — crypto F&G used as a cross-asset proxy.
        #                         Only returned when every FX-native source
        #                         fails. Scored at HALF weight because no
        #                         validated crypto↔FX correlation exists in
        #                         this codebase.
        #   3. "fallback"       — static neutral 50; score forced to 0.
        #
        # The operator can also force halving off via SENTIMENT_FG_SOURCE=fx_native
        # if they wire in their own FX-native source upstream of this engine.
        import os as _os
        ...
        if not is_fx_native and score != 0:
            score = round(score * 0.5)
            desc += " (halved: crypto F&G used as unvalidated FX proxy — source=alternative.me)"
        ...
        if source == "alternative.me":
            ...
            # "fx_native" or any other genuine FX source → verified
            data_quality = "verified"
