...
class EntrySafetyFilters:
    ...
    def liquidity_sweep_filter(direction: str, liquidity_ctx: Optional[Dict[str, Any]]) -> SafetyCheckResult:
        ...
        # Schema-drift guard: this exact filter was silently disabled for
        # months because liquidity_ctx's producer changed key names and
        # nothing here noticed (liquidity_valid stayed falsy -> always
        # "allowed=True" -> no error, no log, no signal anything was wrong).
        # If liquidity_ctx is non-empty but doesn't even contain the
        # 'liquidity_valid' key at all (as opposed to containing it with a
        # False/None value, which is a legitimate "no data this cycle"
        # case), that means the producer's schema has drifted again --
        # log loudly so this is visible in the next run instead of only
        # discoverable via a manual code audit.
        if liquidity_ctx and "liquidity_valid" not in liquidity_ctx:
            log.warning(
                "[EntrySafetyFilters] liquidity_ctx is non-empty but has no "
                "'liquidity_valid' key -- liquidity_sweep_filter's expected "
                f"schema does not match the producer's output. Keys seen: "
                f"{sorted(liquidity_ctx.keys())}. This filter will pass "
                "every trade unchecked until the schema is realigned."
            )
