...
class DecisionValidator:
    ...
    def __init__(self):
        # 2026-07-25: OllamaValidator is now OPT-IN (default disabled).
        # The user requested Ollama be completely removed from the system
        # ("Ollama akebare sese"). When OLLAMA_VALIDATOR_ENABLED is not
        # explicitly "true", we skip instantiating the validator entirely
        # so no localhost:11434 connection is ever attempted.
        self._ollama = None
        try:
            _ollama_enabled = os.getenv("OLLAMA_VALIDATOR_ENABLED", "false").lower() in ("true", "1", "yes")
            if _ollama_enabled:
                from core.ollama_validator import get_ollama_validator
                self._ollama = get_ollama_validator()
            else:
                log.debug("[DecisionValidator] Ollama validator disabled (set OLLAMA_VALIDATOR_ENABLED=true to enable)")
