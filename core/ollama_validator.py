...
# 2026-07-25: OLLAMA_VALIDATOR_ENABLED is now DEFAULT FALSE.
# The user explicitly requested Ollama be completely removed from the
# system ("Ollama akebare sese"). The OllamaValidator is a separate
# concern from the AIAnalyst/MasterAnalyst LLM cascade — it's an
# institutional-grade final veto gate. It is kept as an OPT-IN feature:
# set OLLAMA_VALIDATOR_ENABLED=true in .env to re-enable. When disabled,
# DecisionValidator simply skips the ollama_check (see core/decision_validator.py).
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "http://localhost:11434")
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "qwen3:4b")
OLLAMA_TIMEOUT_SEC = int(os.getenv("OLLAMA_TIMEOUT_SEC", "15"))
OLLAMA_ENABLED = os.getenv("OLLAMA_VALIDATOR_ENABLED", "false").lower() in ("true", "1", "yes")
