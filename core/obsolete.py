...
OBSOLETE_MODULES: List[ObsoleteEntry] = [
    ...
    ObsoleteEntry(
        ...
        ObsoleteStatus.DEAD,
        ArchiveState.NO_ARCHIVE,
        'Day-61 master SMC orchestrator. Composed structure + liquidity + '
        'order_block + fvg_detector — but `from analysis.liquidity import '
        'LiquidityEngine` (line 1 of this file) raises ImportError at '
        'runtime because the class was renamed to LiquidityPoolAnalyzer '
        '(see analysis/liquidity.py header). Never imported by '
        'agents/analysis_agent.py (which uses the Day-62 '
        'analysis.smc_engine.SMCEngine + analysis.smc_advanced.SMCAdvancedEngine '
        'pair instead). Zero live importers + broken import = dead duplicate '
        'of the SMC functionality that IS wired (smc_engine + smc_advanced).',
        'Delete or move to legacy/. Dead duplicate of wired SMC modules.',
        Confidence.HIGH,
        evidence='Execution-proof audit (ANALYSIS-CATALOG §3 "Duplicate clusters"): '
                 'grep -rEn "from analysis.smart_money" returns ZERO matches in '
                 'production paths; only consumer was the (also-dead) hybrid/flow_controller.',
    ),
    ObsoleteEntry(
    ...
    ObsoleteEntry(
        ...
        ObsoleteStatus.DEAD,
        ArchiveState.NO_ARCHIVE,
        'DAY-62 RENAME: this file used to export `LiquidityEngine`, but the '
        'class was renamed to `LiquidityPoolAnalyzer` to avoid a name clash '
        'with `analysis/liquidity_engine.py` (the Day-62 orchestrator that '
        'composes liquidity_zones + session_analysis + stop_hunt_detector + '
        'fvg_detector + liquidity_structure). The previous obsolete entry '
        'claimed this file was ACTIVE because analysis_agent.py did '
        '`from analysis.liquidity import LiquidityEngine` — but that import '
        'BROKE when the class was renamed and was silently caught by a '
        'try/except, leaving liquidity_ctx = {} on every cycle. The '
        'WIRING FIX at agents/analysis_agent.py:380-401 changed the import '
        'to `from analysis.liquidity_engine import LiquidityEngine` (the '
        'Day-62 orchestrator), so this file no longer has ANY live importer. '
        'Safe to delete or move to legacy/.',
        'Delete or move to legacy/. Zero live importers post-rename.',
        Confidence.HIGH,
        evidence='Execution-proof audit (CONSUMPTION-MAP): grep -rEn "from analysis.liquidity import" '
                 'returns ZERO matches in production paths. The file\'s own header '
    ...
    ObsoleteEntry(
        ...
        ObsoleteStatus.ACTIVE,
        ArchiveState.NOT_APPLICABLE,
        'Day-62 liquidity orchestrator. Composes liquidity_zones + '
        'session_analysis.LondonManipulationDetector + stop_hunt_detector '
        '+ fvg_detector + liquidity_structure. Wired via TWO paths: '
        '(1) Direct import in agents/analysis_agent.py:382 (Step 2b of '
        'AnalysisAgent.run); (2) Registered in core/_orphan_integration.py:749 '
        'and resolved per-cycle by core/orphan_consumers.py::enrich_market_context(). '
        'Output flows into liquidity_ctx, which is consumed by '
        'core/entry_safety_filters.py::liquidity_sweep_filter, '
        'risk/trade_permission.py, and risk/institutional_entry_framework.py.',
        'ACTIVE — do not delete. The previous obsolete entry that marked '
        'this DEAD was INVERTED with analysis/liquidity.py — that error '
        'would have caused an operator to delete the live module.',
        Confidence.HIGH,
        evidence='Execution-proof audit (TRADER-MAP + CONSUMPTION-MAP): '
                 'verified two independent consumption paths via grep + '
                 'line-by-line read of analysis_agent.py + orphan_consumers.py.',
    ),
    ObsoleteEntry(
        'analysis/liquidity_zones.py',
        ObsoleteStatus.ACTIVE_SUPPORT,
        ArchiveState.NOT_APPLICABLE,
        'CORRECTED (execution-proof audit): previous entry claimed DEAD with '
        '"only consumer was dead liquidity_engine.py" — but liquidity_engine.py '
        'is itself ACTIVE (see corrected entry above). LiquidityZoneMapper is '
        'instantiated inside LiquidityEngine.analyze() (transitive dependency). '
        'Also registered directly in core/_orphan_integration.py:767 as '
        '`liquidity_zones` service. The previous "DEAD" status was a '
        'cascade error from the inverted liquidity.py/liquidity_engine.py entries.',
        'ACTIVE_SUPPORT — do not delete. Transitive dep of live LiquidityEngine.',
        Confidence.HIGH,
        evidence='Execution-proof audit (ANALYSIS-CATALOG): grep -rEn "from analysis.liquidity_zones" '
                 'returns matches inside analysis/liquidity_engine.py; also registered in '
                 'core/_orphan_integration.py:767.',
    ),
    ObsoleteEntry(
        'analysis/stop_hunt_detector.py',
        ObsoleteStatus.ACTIVE_SUPPORT,
        ArchiveState.NOT_APPLICABLE,
        'CORRECTED (execution-proof audit): same cascade error as '
        'liquidity_zones.py. Imported by the ACTIVE analysis/liquidity_engine.py '
        '(transitive dependency of the live LiquidityEngine orchestrator). '
        'The previous "DEAD" status was inherited from the inverted '
        'liquidity.py/liquidity_engine.py entries.',
        'ACTIVE_SUPPORT — do not delete. Transitive dep of live LiquidityEngine.',
        Confidence.HIGH,
        evidence='Execution-proof audit (ANALYSIS-CATALOG): grep -rEn "from analysis.stop_hunt_detector" '
                 'returns matches inside analysis/liquidity_engine.py.',
    ),
    ObsoleteEntry(
        'analysis/session_analysis.py',
        ObsoleteStatus.ACTIVE_SUPPORT,
        ArchiveState.NOT_APPLICABLE,
        'CORRECTED (execution-proof audit): LondonManipulationDetector is '
        'imported by the ACTIVE analysis/liquidity_engine.py (transitive '
        'dependency). Also registered directly in core/_orphan_integration.py:904 '
        'as `session_analysis` service. The previous "DEAD" status was a '
        'cascade error from the inverted liquidity.py/liquidity_engine.py entries. '
        'This file is DISTINCT from analysis/session_analyzer.py (the Day-63 '
        'SessionAnalyzer that runs as Step 0/Day-63 re-run in AnalysisAgent) — '
        'BOTH are live, serving different roles.',
        'ACTIVE_SUPPORT — do not delete. Transitive dep of live LiquidityEngine; '
        'also registered as orphan service.',
        Confidence.HIGH,
        evidence='Execution-proof audit (ANALYSIS-CATALOG): grep -rEn "from analysis.session_analysis" '
                 'returns matches inside analysis/liquidity_engine.py; also registered '
                 'in core/_orphan_integration.py:904.',
    ),
    ObsoleteEntry(
